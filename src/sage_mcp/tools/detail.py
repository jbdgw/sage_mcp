...
async def get_product_detail(
    ...
    include_images: Annotated[
        bool, Field(description="Include the pics array (set False to trim payload)")
    ] = True,
    *,
    ctx: Context,
    ...
    Returns pricing tiers (catalog + net), decoration/setup charges, shipping
    cartons, LIVE INVENTORY (onHand + per-variant skus), supplier info, and images
    — no separate check_inventory call needed after this.
    """
    client: SageClient = ctx.lifespan_context["sage_client"]
    try:
        resp = await client.get_product_detail(
            prod_eid=prod_eid,
